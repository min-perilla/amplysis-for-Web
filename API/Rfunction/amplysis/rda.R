...
When `Axis lengths` value < 3.0, choose RDA;\n\033[0m")
  ...
  ##
  # vegan 包 rda 函数支持 RDA 分析
  df_rda <- vegan::rda(t(otu6), env6, scale = T)


  ## 提取绘图数据
  df_rda2 <- data.frame(df_rda$CCA$u[, 1:2], rownames(env6))                # 提取数据
  colnames(df_rda2) = c("RDA1", "RDA2", "sample")                       # 重命名列名
  df_rda2 <- dplyr::left_join(df_rda2, metadata3, by = c("sample" = "sample"))  # 将绘图数据和分组合并


  ## 将数据 df_rda2 中的分组列名统一为 “group”
  ...
  ##
  #提取其他数据
  df_rda_score <- data.frame(df_rda$CCA$v[,1:2])            # 提取物种得分
  df_rda_env <- as.data.frame(df_rda$CCA$biplot[,1:2])      # 提取环境因子得分
  ...
  # cca_sum <- summary(df_rda)  # 描述统计
  # 检验环境因子相关显著性（Monte Carlo permutation test）
  # df_permutest <- permutest(df_rda, permu = 999) # permu = 999 是表示置换循环的次数
  # 每个环境因子显著性检验
  ...
  result <- list(
    ...
    RDA1 = RDA1,           # 轴特征值 1
    RDA2 = RDA2,           # 轴特征值 2
    score = df_rda_score,  # 提取物种得分
    env = df_rda_env,      # 提取环境因子得分

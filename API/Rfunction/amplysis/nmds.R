# 如果没有安装必要的包，则先安装
required_packages <- c("vegan", "tidyr", "dplyr")
...
nmds <- function(otu,                # otu 表格
  ...
  otu.distance <- vegan::vegdist(otu_t, method = "bray")        #计算 bray_curtis 距离
  
  df_NMDS <- vegan::metaMDS(otu.distance, k = 2, trymax = 20)   # NMDS 排序分析，关注 stress、points 及 species 三个指标
  Stress <- df_NMDS$stress
  cat("一般情况下，Stress值 ＜ 0.2 可用；＜ 0.1 效果较好\n",
      "Stress = ", Stress,  
      if(Stress < 0.2 && Stress > 0.1) {
        paste0("\033[34m", "（可用）", "\033[39m")
      } else if(Stress < 0.1) {
        paste0("\033[32m", "（效果较好）", "\033[39m")
      } else {
        paste0("\033[31m", "（较差）", "\033[39m")
      }, "\n", sep = "")
  vegan::stressplot(df_NMDS)  # 检查观测值非相似性与排序距离之间的关系，没有点分布在线段较远位置表示该数据可以使用 NMDS 分析
  
  
  #绘图数据
  NMDS <- as.data.frame(df_NMDS$points)                    # 提取作图数据
  NMDS <- cbind(sample = rownames(NMDS), NMDS)             # 提取行名作为单独的一列
  NMDS <- merge(NMDS, metadata3, by = "sample", sort = F)  # 将绘图数据和分组合并
  
  # 重命名列名
  colnames(NMDS)[colnames(NMDS) == group] <- "group"
  
  # 返回结果
  result <- list(NMDS = NMDS,     # 绘图数据
                 stress = Stress  # 应力值
  )                       
  return(result)
}

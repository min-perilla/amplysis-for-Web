...
source("./Rfunction/amplysis/CCA.R")       # 分析函数
source("./Rfunction/amplysis/CCA_plot.R")  # 绘图函数
source("./Rfunction/amplysis/process_metadata.R")  # 处理 metadata 函数


# ------------------------------------------------------------------------------
api_cca_plot <- function(req) {
  # 解析请求体中的 JSON 数据
  ...
  # 提取方法
  ellipse_type <- body$methods$ellipse_type
  cat("ellipse_type", "(", class(ellipse_type), ")", ": ", ellipse_type, "\n", sep = "")
  ...
  if ((is.null(otu) || nrow(otu) == 0 || ncol(otu) == 0) || 
    ...
    if (is.null(env) || nrow(env) == 0 || ncol(env) == 0) {
      message("错误: Env 表为空或没有有效数据。")
      env <- -1
    }
  ...
  p = CCA_plot(
    ...
    ellipse_type = ellipse_type,  # 置信椭圆的计算方法，可选三种：
